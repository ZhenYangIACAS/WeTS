...
def collate(
    ...
    input_feeding=True, src_segment_id=0, mask_src_segment_id=1,
):
    if len(samples) == 0:
        return {}
    ...
    src_tokens, src_positions, src_segments  = merge_two('source', 
                                                         ...
                                                         segment2_id=mask_src_segment_id)
...
class LanguageTripleDataset(FairseqDataset):
    ...
    def __init__(
        ...
        mask_src, mask_src_sizes, mask_src_dict,
        tgt=None, tgt_sizes=None, tgt_dict=None,
        left_pad_source=True, left_pad_target=False,
        max_source_positions=1024, max_target_positions=1024,
        shuffle=True, input_feeding=True, remove_eos_from_source=False, append_eos_to_target=False,
        src_segment_id=1, mask_src_segment_id=2,
    ):
        if tgt_dict is not None:
            assert src_dict.pad() == tgt_dict.pad()
            assert src_dict.eos() == tgt_dict.eos()
            assert src_dict.unk() == tgt_dict.unk()
            assert mask_src_dict.pad() == tgt_dict.pad()
            assert mask_src_dict.eos() == tgt_dict.eos()
            assert mask_src_dict.unk() == tgt_dict.unk()
        ...
        self.mask_src_sizes = np.array(mask_src_sizes)
        self.tgt_sizes = np.array(tgt_sizes) if tgt_sizes is not None else None
        self.src_dict = src_dict
        self.mask_src_dict = mask_src_dict
        ...
        self.mask_src_segment_id = mask_src_segment_id

    def __getitem__(self, index):
        tgt_item = self.tgt[index] if self.tgt is not None else None
        src_item = self.src[index]
        mask_src_item = self.mask_src[index]
        ...
        if self.remove_eos_from_source:
            ...
            if self.mask_src[index][-1] == eos:
                mask_src_item = self.mask_src[index][:-1]
    ...
    def collater(self, samples):
        ...
        return collate(
            ...
            mask_src_segment_id = self.mask_src_segment_id,
        )

    def num_tokens(self, index):
        """Return the number of tokens in a sample. This value is used to
        enforce ``--max-tokens`` during batching."""
        return max(self.src_sizes[index] + self.mask_src_sizes[index], self.tgt_sizes[index] if self.tgt_sizes is not None else 0)

    def size(self, index):
        """Return an example's size as a float or tuple. This value is used when
        filtering a dataset with ``--max-positions``."""
        return (self.src_sizes[index] + self.mask_src_sizes[index], self.tgt_sizes[index] if self.tgt_sizes is not None else 0)
    ...
    def ordered_indices(self):
        ...
        combine_src_sizes = [self.src_sizes[idx] + self.mask_src_sizes[idx] for idx in indices]

        return indices[np.argsort(combine_src_sizes, kind='mergesort')]

    @property
    def supports_prefetch(self):
        return (
            getattr(self.src, 'supports_prefetch', False)
            and getattr(self.mask_src, 'supports_prefetch', False)
            and (getattr(self.tgt, 'supports_prefetch', False) or self.tgt is None)
        )

    def prefetch(self, indices):
        self.src.prefetch(indices)
        self.mask_src.prefetch(indices)
