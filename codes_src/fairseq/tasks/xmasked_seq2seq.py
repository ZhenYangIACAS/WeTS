...
class XMassTranslationTask(FairseqTask):
    ...
    def load_dataset(self, split, **kwargs):
        ...
        for lang_pair in self.args.mono_lang_pairs:
            ...
            src_mono_datasets[lang_pair] = data_utils.load_indexed_dataset(prefix, self.dicts[lang], self.args.dataset_impl)
        ...
        for lang_pair in self.args.para_lang_pairs:
            ...
            if split_para_exists(split, px, src) and split_para_exists(split, px, tgt):
                prefix = os.path.join(self.args.data, '{}.{}'.format(split, px))
            elif split_para_exists(split, px_r, src) and split_para_exists(split, px_r, tgt):
                prefix = os.path.join(self.args.data, '{}.{}'.format(split, px_r)) 
            else:
                raise FileNotFoundError('Not Found available {}-{} para dataset for ({}, {}) lang'.format(split, key, src, tgt))
            
            if '{}.{}'.format(key, src) not in src_para_datasets:
                # src_para_datasets[key + '.' + src] = indexed_dataset(prefix + '.' + src, self.dicts[src])
                src_para_datasets[key + '.' + src] = data_utils.load_indexed_dataset(prefix + '.' + src, self.dicts[src], self.args.dataset_impl)
            if '{}.{}'.format(key, tgt) not in src_para_datasets:
                # src_para_datasets[key + '.' + tgt] = indexed_dataset(prefix + '.' + tgt, self.dicts[tgt])
                src_para_datasets[key + '.' + tgt] = data_utils.load_indexed_dataset(prefix + '.' + tgt, self.dicts[tgt], self.args.dataset_impl)
    ...
    def train_step(self, sample, model, criterion, optimizer, ignore_grad=False):
        ...
        def forward_backward(model, samples, logging_output_key, lang_pair, weight=1.0):
            ...
            # for debug
            #for i, id in enumerate(samples['id'].tolist()):
            #    src_tokens = utils.strip_pad(samples['net_input']['src_tokens'][i,:],
            #    self.dicts[src_key].pad())
            #    print('|debug during training src_str {}'.format(self.dicts[src_key].string(src_tokens, args.remove_bpe)))
