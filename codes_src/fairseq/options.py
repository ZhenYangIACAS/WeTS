...
def add_dataset_args(parser, train=False, gen=False):
    ...
    if train:
        ...
        # added by zieenyang 2019/08/06

        group.add_argument('--bleu-valid-script', type=str, metavar='SPLIT',
                           help='script for BLEU valid process')
        group.add_argument('--bleu-valid-src-file', type=str, metavar='SPLIT',
                           help='source file for BLEU valid process')
        group.add_argument('--bleu-valid-trg-file', type=str, metavar='SPLIT',
                           help='target file for BLEU valid process')
        group.add_argument('--bleu-valid-src-lang', type=str, metavar='SPLIT',
                           help='source language for valid bleu')
        group.add_argument('--bleu-valid-trg-lang', type=str, metavar='SPLIT',
                           help='target language for valid bleu')
    if gen:
...
def add_checkpoint_args(parser):
    ...
    group.add_argument('--patience', type=int, default=-1, metavar='N',
                       help='patience for early stop the training')
    # fmt: on
    return group
...
def add_interactive_args(parser):
    ...
    group.add_argument('--input', default='-', type=str, metavar='FILE', nargs="+",
                       help='file to read from; use - for stdin')
    group.add_argument('--seperate-input', action='store_true', help='use seperate inputs')
    # fmt: on
...
def add_model_args(parser):
    ...
    group.add_argument('--init-type', default='default', type=str, choices=['default', 'adaptive', 'adaptive-profiling'])
    group.add_argument('--profile-ratio-file', default='', type=str)
    group.add_argument('deep-init', action='store_true', help='enable deep init for deep transformer')
    # fmt: on
    return group
