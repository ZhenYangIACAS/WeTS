...
from fairseq.modules import (
    ...
    LayerNorm,
    MultiheadAttention,
    SegmentAwareMultiheadAttention,
    PositionalEmbedding,
...
encoder_ratio = 1.0
decoder_ratio = 1.0
profile_ratio_handle = 0

model_arch='istransformer'

@register_model(model_arch)
class ISTransformerModel(FairseqEncoderDecoderModel):
    """
    ...
    def add_args(parser):
        ...
        #parser.add_argument('--deep-init', action='store_true',
        #                    help='enable deep init for deep transformer')
    ...
    def build_model(cls, args, task):
        ...
        return ISTransformerModel(encoder, decoder)

    @classmethod
    def build_encoder(cls, args, src_dict, embed_tokens):
        return ISTransformerEncoder(args, src_dict, embed_tokens)

    @classmethod
    def build_decoder(cls, args, tgt_dict, embed_tokens):
        return ISTransformerDecoder(args, tgt_dict, embed_tokens)


class ISTransformerEncoder(FairseqEncoder):
    """
    ...
    def __init__(self, args, dictionary, embed_tokens, shared_encoder_layers=None):
        ...
        self.embed_positions = (
            nn.Embedding(args.max_source_positions, embed_dim, padding_idx=0) 
            if not args.no_token_positional_embeddings 
            else None
        )

        self.segment_embeddings = (
            nn.Embedding(self.num_segments, embed_dim, padding_idx=0)
        ...
        if shared_encoder_layers is None:
            self.layers.extend([
                TransformerEncoderLayer(args, (i+1) ** -0.5) if args.deep_init
                else TransformerEncoderLayer(args)
    ...
    def forward(self, src_tokens, src_lengths, src_segments=None, src_positions=None):
        """
        Args:
        ...
        if self.embed_positions is not None:
            if src_positions is not None:
                x += self.embed_positions(src_positions)
            else:
                x += self.embed_positions(src_tokens)

        segment_emb=None
        if self.segment_embeddings is not None and src_segments is not None:
            segment_emb = self.segment_embeddings(src_segments)
            x += segment_emb
        ...
        for layer in self.layers:
            x = layer(x, encoder_padding_mask, segment_emb)
    ...
    def max_positions(self):
        ...
        return min(self.max_source_positions, self.max_source_positions)
...
class ISTransformerDecoder(FairseqIncrementalDecoder):
    """
    ...
    shared_decoder_layers=None):
        ...
        if shared_decoder_layers is None:
            self.layers.extend([
                TransformerDecoderLayer(args, no_encoder_attn, initialize_scale=(i+1) ** -0.5) if args.deep_init
                else TransformerDecoderLayer(args, no_encoder_attn)
            ...
            self.layers.extend([
                TransformerDecoderLayer(args, no_encoder_attn, 
                     ...
                     if args.deep_init else TransformerDecoderLayer(args, no_encoder_attn)
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    Updates: 2020/11/04
    By: zieenyang
    Attributes: enabling adaptive-profiling initialization(paper:https://arxiv.org/abs/2008.07772)
    """

    def __init__(self, args, initialize_scale=None, layer_num=None):
        super().__init__()
        global profile_ratio_handle
        self.embed_dim = args.encoder_embed_dim

        self.args = args
        self.initialize_scale = initialize_scale
        self.layer_num = layer_num
        self.normalize_before = args.encoder_normalize_before
        self.embed_dim = args.encoder_embed_dim

        self.self_attn = SegmentAwareMultiheadAttention(
            self.embed_dim, args.encoder_attention_heads,
            dropout=args.attention_dropout, self_attention=True,
            initialize_scale=self.initialize_scale
        )
        ...
        if 'adaptive' in args.init_type:
            ...
            if 'adaptive-profiling' == args.init_type:
                if not profile_ratio_handle:
                    if args.profile_ratio_file:
                        profile_ratio_handle = open(args.profile_ratio_file, 'w')
                    else:
                        profile_ratio_handle = open('profile.ratio.init', 'w')
                ...
                if not profile_ratio_handle:
                    if args.profile_ratio_file:
                        profile_ratio_handle = open(args.profile_ratio_file, 'r')
                    else:
                        profile_ratio_handle = open('profile.ratio.init', 'r')

                layer_iter, next_value = [float(tup) for tup in profile_ratio_handle.readline().split()]
    ...
    def forward(self, x, encoder_padding_mask, segment_emb=None):
        """
        Args:
        ...
        x, _ = self.self_attn(query=x, key=x, value=x, key_padding_mask=encoder_padding_mask, segment_bias=segment_emb)
        x = F.dropout(x, p=self.dropout, training=self.training)

        if 'adaptive' in self.args.init_type:
            if not_initialized:
                global encoder_ratio, profile_ratio_handle
                tmp_layer_ind = self.layer_num * 2 + 1
                tmp_weight = 1.
                tmp_ratio = encoder_ratio / tmp_weight
                profile_ratio_handle.write('{} {}\n'.format(tmp_layer_ind, tmp_ratio))
...
class TransformerDecoderLayer(nn.Module):
    ...
    *args.decoder_normalize_before* to ``True``.
    ...
    def __init__(self, args, no_encoder_attn=False, add_bias_kv=False, add_zero_attn=False, initialize_scale=None, layer_num=None):
        super().__init__()
        self.initialize_scale = initialize_scale
        ...
        if 'adaptive' in args.init_type:
            ...
            if 'adaptive-profiling' == args.init_type:
                if not profile_ratio_handle:
                    if args.profile_ratio_file:
                        profile_ratio_handle = open(args.profile_ratio_handle, 'w')
                    else:
                        profile_ratio_handle = open('profile.ratio.init', 'w')
        ...
        self.fc1 = Linear(self.embed_dim, args.decoder_ffn_embed_dim, initialize_scale=self.initialize_scale)
        self.fc2 = Linear(args.decoder_ffn_embed_dim, self.embed_dim, initialize_scale=self.initialize_scale)
    ...
    def forward(
        ...
        if 'adaptive' in self.args.init_type:
            if not_initialized:
                global decoder_ratio, profile_ratio_handle
...
# modified by zieenyang 20190918
# enable deep init
def Linear(in_features, out_features, bias=True, initialize_scale=None):
    m = nn.Linear(in_features, out_features, bias)
    if not initialize_scale:
        nn.init.xavier_uniform_(m.weight)
    else:
        nn.init.xavier_uniform_(m.weight, initialize_scale)
    if bias:
        nn.init.constant_(m.bias, 0.)
    return m


@register_model_architecture('istransformer', 'istransformer')
def base_architecture(args):
    ...
    args.deep_init = getattr(args, 'deep_init', False)
    ...
    args.src_segment_nums = getattr(args, 'src_segment_nums', 3)

@register_model_architecture('istransformer', 'istransformer_iwslt_de_en')
...
@register_model_architecture('istransformer', 'istransformer_wmt_en_de')
def transformer_wmt_en_de(args):
    base_architecture(args)


# parameters used in the "Attention Is All You Need" paper (Vaswani et al., 2017)
@register_model_architecture('istransformer', 'istransformer_vaswani_wmt_en_de_big')
...
@register_model_architecture('istransformer', 'istransformer_vaswani_wmt_en_fr_big')
def transformer_vaswani_wmt_en_fr_big(args):
    args.dropout = getattr(args, 'dropout', 0.1)
    transformer_vaswani_wmt_en_de_big(args)


@register_model_architecture('istransformer', 'istransformer_wmt_en_de_big')
...
@register_model_architecture('istransformer', 'istransformer_wmt_en_de_big_t2t')
...
@register_model_architecture('istransformer', 'istransformer_deep')
...
@register_model_architecture('istransformer', 'istransformer_deep_20_6')
def transformer_deep(args):
